...
public class ProjectService {
  ...
  public Project addProject(String projectName) {
    ...
    // 중복 프로젝트 이름 확인
    Optional<Project> existingProject = projectRepository.findByNameAndOwner(
      projectName,
      user
    );
    ...
    // 새 프로젝트 생성
    Project project = Project.builder().name(projectName).owner(user).build();
    project = projectRepository.save(project);
    // root 폴더 생성
    /* 1. id : seq
     *    name : root
     *    project는 생성 후 id 받아와서 동작
     */

    folderRepository.save(
      Folder.builder().name("root").project(project).build()
    );
    return project;
  }
